...
namespace LogSuite.Client.Parsers
{
    public class ExcelDailyReviewParser
    {
        ...
        public async Task ParseFile()
        {
            ...
            else if (fileSettings.Name.Equals("Фактические поставки"))
            {
                var dateReport = StringParser.GetFirstDateFromString(filename);
                if (dateReport == null)
                {
                ...
                ExcelFactSupplyParser parser = new ExcelFactSupplyParser(_file, _gisList, fileSettings, _toastService);
                valueList = await parser.GetResult();
            }
            else if (fileSettings.Name.Equals("Тетеревка"))
            {
                var dateReport = StringParser.GetFirstDateFromString(filename);
                if (dateReport == null)
                {
                ...
                ExcelTeterevkaParser parser = new ExcelTeterevkaParser(_file, _gisList, fileSettings, _toastService);
                valueList = await parser.GetResult();
            }
        }
