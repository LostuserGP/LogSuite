...
namespace LogSuite.Client.Services.DailyReview
{
    public class GisAddonValueService : IGisAddonValueService
    {
        ...
        private const string ApiUrl = "api/gisaddonvalue";

        public GisAddonValueService(HttpClient client, NotificationService notificationService)
        {
            _client = client;
        ...
        public async Task<GisAddonValueDTO> GetOnDateByGisAddonId(int gisAddonId, DateOnly date)
        {
            ...
            var response = await _client.GetAsync(QueryHelpers.AddQueryString($"{ApiUrl}/bygisaddonidondate/{gisAddonId}", queryStringParam));
            var result = await response.Content.ReadAsStringAsync();
            if (!response.IsSuccessStatusCode)
            {
                var errorModel = JsonConvert.DeserializeObject<ErrorModel>(result);
        ...
        public async Task<IEnumerable<GisAddonValueDTO>> GetOnDateRangeByGisAddonId(int gisAddonId, DateOnly dateStart, DateOnly dateEnd)
        {
            var queryStringParam = new Dictionary<string, string>
            {
            ...
            var response = await _client.GetAsync(QueryHelpers.AddQueryString($"{ApiUrl}/bygisaddonidondaterange/{gisAddonId}", queryStringParam));
            var result = await response.Content.ReadAsStringAsync();
            if (!response.IsSuccessStatusCode)
            {
                var errorModel = JsonConvert.DeserializeObject<ErrorModel>(result);
        ...
        public async Task<PagingResponse<GisAddonValueDTO>> GetPagedByGisAddonId(int gisAddonId, Params parameters)
        {
            ...
            var response = await _client.GetAsync(QueryHelpers.AddQueryString($"{ApiUrl}/bygisaddonidpaged/{gisAddonId}", queryStringParam));
            if (!response.IsSuccessStatusCode)
            {
                var result = await response.Content.ReadAsStringAsync();
                var errorModel = JsonConvert.DeserializeObject<ErrorModel>(result);
