...
{
    public class GisNameService : IGisNameService
    {
        private readonly HttpClient _client;
        private readonly ToastService _toastService;
        private readonly string _apirUrl = "api/gisname";

        public GisNameService(HttpClient client, ToastService toastService)
        {
            _client = client;
        ...
        public async Task<GisNameDTO> Create(GisNameDTO dto)
        {
            ...
            if (response.IsSuccessStatusCode)
            {
                ...
                var answer = JsonConvert.DeserializeObject<GisNameDTO>(result);
                return answer;
            }
            else
            {
        ...
        public async Task<PagingResponse<GisNameDTO>> GetAllByGisId(int gisId, Params parameters)
        {
            ...
            var pagingResponse = new PagingResponse<GisNameDTO>
            {
                Items = System.Text.Json.JsonSerializer.Deserialize<List<GisNameDTO>>(content, new JsonSerializerOptions { PropertyNameCaseInsensitive = true }),
        ...
        public async Task<List<GisNameDTO>> GetAllByGisId(int gisId)
        {
            var response = await _client.GetAsync($"{_apirUrl}/all/{gisId}");
            var content = await response.Content.ReadAsStringAsync();
            var answers = JsonConvert.DeserializeObject<List<GisNameDTO>>(content);
            return answers;
        }
    }
}
