...
{
    public class GisAddonNameService : IGisAddonNameService
    {
        private readonly HttpClient _client;
        private readonly ToastService _toastService;
        private readonly string _apirUrl = "api/gisaddonname";

        public GisAddonNameService(HttpClient client, ToastService toastService)
        {
            _client = client;
        ...
        public async Task<GisAddonNameDTO> Create(GisAddonNameDTO dto)
        {
            ...
            if (response.IsSuccessStatusCode)
            {
                ...
                var answer = JsonConvert.DeserializeObject<GisAddonNameDTO>(result);
                return answer;
            }
            else
            {
        ...
        public async Task<IEnumerable<GisAddonNameDTO>> GetAll()
        {
            ...
            var answers = JsonConvert.DeserializeObject<IEnumerable<GisAddonNameDTO>>(content);
            return answers;
        }

        public async Task<IEnumerable<GisAddonNameDTO>> GetAllByGisAddonId(int gisAddonId)
        {
            var response = await _client.GetAsync($"{_apirUrl}/bygisaddonid/{gisAddonId}");
            var content = await response.Content.ReadAsStringAsync();
            var answers = JsonConvert.DeserializeObject<IEnumerable<GisAddonNameDTO>>(content);
            return answers;
        }

        public async Task<PagingResponse<GisAddonNameDTO>> GetPaged(Params parameters)
        {
            ...
            var pagingResponse = new PagingResponse<GisAddonNameDTO>
            {
                Items = System.Text.Json.JsonSerializer.Deserialize<List<GisAddonNameDTO>>(content, new JsonSerializerOptions { PropertyNameCaseInsensitive = true }),
        ...
        public async Task<PagingResponse<GisAddonNameDTO>> GetPagedByGisAddonId(int gisAddonId, Params parameters)
        {
            var queryStringParam = StringParser.ParamsToDict(parameters);
            var response = await _client.GetAsync(QueryHelpers.AddQueryString($"{_apirUrl}/bygisaddonid/{gisAddonId}/paged", queryStringParam));
            var content = await response.Content.ReadAsStringAsync();
            if (!response.IsSuccessStatusCode)
            {
                throw new ApplicationException(content);
