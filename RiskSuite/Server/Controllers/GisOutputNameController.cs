...
namespace LogSuite.Server.Controllers
{
    ...
    public class GisOutputNameController : Controller
    {
        private IGisOutputNameRepository _repository;

        public GisOutputNameController(IGisOutputNameRepository repository)
        {
        ...
        public async Task<IActionResult> Get(int? id)
        {
            if (id == null)
            {
                ...
                {
                    Title = "",
                    ErrorMessage = "Invalid gis output name Id",
        ...
        [HttpPost]
        public async Task<IActionResult> Create([FromBody] GisOutputNameDTO dto)
        {
            if (ModelState.IsValid)
            {
                ...
                else
                {
                    ...
                    {
                        Title = "",
                        ErrorMessage = "Gis output name with such fields already exist",
            ...
            else
            {
                return BadRequest(new ErrorModel()
                {
                    ErrorMessage = "Error while creating new gis output name"
                });
            }
        }

        [HttpPut("{id}")]
        public async Task<IActionResult> Update([FromBody] GisOutputNameDTO dto, int? id)
        {
            ...
            else
            {
                return BadRequest(new ErrorModel()
                {
                    ErrorMessage = "Error while updating gis output name"
                });
            }
        }
        ...
        public async Task<IActionResult> Delete(int? id)
        {
            ...
            if (result == -1)
            {
                ...
                {
                    Title = "",
                    ErrorMessage = "Can't delete gis output name",
