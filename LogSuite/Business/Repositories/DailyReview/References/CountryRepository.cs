...
namespace LogSuite.Business.Repositories.DailyReview.References
{
    public class CountryRepository : ICountryRepository
    {
        ...
        public async Task<PagedList<CountryDTO>> GetPaged(Params parameters)
        {
            ...
            source = source.Sort(parameters.OrderBy);
            var result = await PagedList<Country>.ToPagedListAsync(source, parameters.Skip, parameters.Top);
        ...
        public async Task<CountryDTO> IsUnique(CountryDTO dto, int id = 0)
        {
            ...
            {
                var fromDb = await _db.Countries
                    .Where(x => string.Equals(x.Name, dto.Name))
                    .FirstOrDefaultAsync();
