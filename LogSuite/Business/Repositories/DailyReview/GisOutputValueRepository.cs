...
namespace LogSuite.Business.Repositories.DailyReview
{
    public class GisOutputValueRepository : IGisOutputValueRepository
    {
        ...
        {
            var toDb = _mapper.Map<GisOutputValueDTO, GisOutputValue>(dto);
        ...
        public Task<IEnumerable<GisOutputValueDTO>> GetAll()
        {
            return null;
        }

        public async Task<GisOutputValueDTO> Get(long id)
        {
            var fromDb = await _db.GisOutputValues
                .Include(x => x.RequestedValueTime)
                .Include(x => x.AllocatedValueTime)
        ...
        public async Task<PagedList<GisOutputValueDTO>> GetPagedByGisId(int gisId, Params parameters)
        {
            ...
            var result = await PagedList<GisOutputValue>.ToPagedListAsync(source, parameters.Skip, parameters.Top);
        ...
        public async Task<List<GisOutputValueDTO>> GetOnDateRangeByGisId(int gisId, DateOnly startDate, DateOnly finishDate)
        {
        ...
        public async Task<GisOutputValueDTO> IsUnique(GisOutputValueDTO dto, long id = 0)
        {
            if (id == 0)
            {
                var fromDb = await _db.GisOutputValues
        ...
        public Task<PagedList<GisOutputValueDTO>> GetPaged(Params parameters)
        {
            return null;
        }
