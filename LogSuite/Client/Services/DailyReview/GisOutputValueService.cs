...
{
    public class GisOutputValueService : IGisOutputValueService
    {
        private readonly HttpClient _client;
        private readonly NotificationService _notificationService;
        private const string ApiUrl = "api/gisoutputvalue";

        public GisOutputValueService(HttpClient client, NotificationService notificationService)
        {
            _client = client;
        ...
        public async Task<GisOutputValueDTO> Create(GisOutputValueDTO dto)
        {
            ...
            if (response.IsSuccessStatusCode)
            {
                ...
                var answer = JsonConvert.DeserializeObject<GisOutputValueDTO>(result);
                return answer;
            }
            else
            {
        ...
        public async Task<GisOutputValueDTO> GetOnDateByGisId(int gisId, DateOnly date)
        {
        ...
        public async Task<IEnumerable<GisOutputValueDTO>> GetOnDateRangeByGisId(int gisId, DateOnly dateStart, DateOnly dateEnd)
        {
            var queryStringParam = new Dictionary<string, string>
            {
            ...
            var answers = JsonConvert.DeserializeObject<IEnumerable<GisOutputValueDTO>>(result);
            return answers;
        }

        public async Task<PagingResponse<GisOutputValueDTO>> GetPagedByGisId(int gisId, Params parameters)
        {
            ...
            var pagingResponse = new PagingResponse<GisOutputValueDTO>
            {
                Items = System.Text.Json.JsonSerializer.Deserialize<List<GisOutputValueDTO>>(content, new JsonSerializerOptions { PropertyNameCaseInsensitive = true }),
