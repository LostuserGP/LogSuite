...
namespace LogSuite.Client.Services.DailyReview
{
    public class GisCountryValueService : IGisCountryValueService
    {
        ...
        private const string ApiUrl = "api/giscountryvalue";

        public GisCountryValueService(HttpClient client, NotificationService notificationService)
        {
            _client = client;
        ...
        public async Task<List<GisCountryValueDTO>> GetOnDateRangeByGisCountryId(int gisCountryId, DateOnly dateStart, DateOnly dateEnd)
        {
            var queryStringParam = new Dictionary<string, string>
            {
            ...
            var response = await _client.GetAsync(QueryHelpers.AddQueryString($"{ApiUrl}/bygiscountryidondaterange/{gisCountryId}", queryStringParam));
            var result = await response.Content.ReadAsStringAsync();
            if (!response.IsSuccessStatusCode)
            {
                var errorModel = JsonConvert.DeserializeObject<ErrorModel>(result);
